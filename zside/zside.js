...
let app = express();
let zside_rpc = require('node-bitcoin-rpc');
...
zside_rpc.init(host, port, user, pass)
zside_rpc.setTimeout(30000) // 30 seconds
*/
...
function dynamicRPCCall(rpcType, method, params) {
  return new Promise((resolve, reject) => {
    const rpc = rpcType === 'bitcoin' ? bitcoin_rpc : zside_rpc;
    const port = rpcType === 'bitcoin' ? 8332 : 8232;
...
app.get('/api/getdepositaddress', function (req, res) {
  dynamicRPCCall('zside', 'getdepositaddress', [])
    .then(rpcRes => {
      console.log(rpcRes);
...
app.get('/api/listactivesidechains', function (req, res) {
  dynamicRPCCall('zside', 'listactivesidechains', [])
    .then(rpcRes => {
      if (typeof rpcRes.result !== 'undefined') {
...
dynamicRPCCall('zside', 'listwallets', [])
  .then(wallets => {
    ...
    wallets.result.forEach(function(walletName) {
      if (walletName !== mostRecentWalletName) {
        dynamicRPCCall('zside', 'unloadwallet', [walletName])
          .then(() => {
...
app.get('/zsideRpc/:rpcMethod', function (req, res) {
  const rpcMethod = req.params.rpcMethod;
  console.log("api button");
  dynamicRPCCall('zside', rpcMethod, [])
    .then(rpcRes => {
      if (typeof rpcRes.result !== 'undefined') {
        if (rpcMethod == 'getblockchaininfo' || rpcMethod == 'getwalletinfo') {
...
app.get('/api/sendtoaddress/:address/:amount', function (req, res) {
  ...
  dynamicRPCCall('zside', 'sendtoaddress', params)
    .then(rpcRes => {
      if (typeof rpcRes.result !== 'undefined') {
        res.send(JSON.stringify(rpcRes.result));
        console.log(rpcRes.result);
...
app.get('/api/loadwallet/:wallet', function (req, res) {
  ...
  const unloadPromises = allWallets.map(function(unloadWallet) {
    return dynamicRPCCall('zside', 'unloadwallet', [unloadWallet])
      .then(() => {
  ...
  Promise.all(unloadPromises)
    .then(() => {
      // Load the new wallet
      return dynamicRPCCall('zside', 'loadwallet', params);
    })
    .then(rpcRes => {
      if (typeof rpcRes.result !== 'undefined') {
        res.send(JSON.stringify(rpcRes.result));
...
app.get('/estimatesmartfee', function (req, res) {
 const blocks = 6;
 const params = [blocks];
 zside_rpc.call('estimatesmartfee', params, function (err, rpcRes) {
   if (err) {
     res.status(500).send({ error: "I have an error :\n" + err });
...
zside_rpc.call('listwallets', [], function (err, wallets) {
   if (err) {
     res.status(500).send({ error: "I have an error :\n" + err });
     return;
   }
...
zside_rpc.call('importprivkey', [keyPair.toWIF(), "", false], function (err, rpcRes) {
...
zside_rpc.call('createwallet', [name, false, true], function (err, rpcRes) {
  if (err) {
    res.status(500).send({ error: "I have an error :\n" + err });
  } else if (typeof rpcRes.result !== 'undefined') {
...
const unloadPromises = allWallets.map(function(unloadWallet) {
 return new Promise((resolve, reject) => {
   zside_rpc.call('unloadwallet', [unloadWallet], function (err, rpcRes) {
     if (err) {
       console.log("Error unloading wallet: " + err);
       reject(err);
     } else {
...
zside_rpc.call('importprivkey', [wif, "", false], function (err, rpcRes) {
 if (err) {
  res.status(500).send({ error: "I have an error :\n" + err });
 } else {
...
// Serve static files from the zside directory
//app.use('/zside', express.static(path.join(__dirname, 'zside')));

// Serve zside.html
app.get('/zside/zside.html', (req, res) => {
  res.sendFile(path.join(__dirname, 'zside.html'));
});

// Your zside-specific routes and logic here...
...
fs.writeFileSync(path.join(parentDir, 'zside_key.txt'), customUrlParam.toString());
*/
...
getPublicIP().then((ip) => {
  ...
  server.listen(port, () => {
    console.log(`HTTPS Server running at https://${ip}:${port}/zside/zside.html`);
